...
- `cms_actions` – text  
  CI-/team-definierte Aktion für dieses Asset, z. B. `keep`, `update`, `consolidate`, `redirect`, `archive`, `review`.

- `cms_actions_updated_at` – timestamp with time zone  
  Zeitpunkt der letzten Aktualisierung von `cms_actions`.

#### 2.1.9 Indexes & Constraints
...
- **Schema Changes:** Added `cms_actions` and `cms_actions_updated_at` fields to `content_inventory` table
